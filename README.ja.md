...
- ホットキーでローカルキーボードを一時ロックします。
- 設定すると、ロック時に画面もオフにします。
- 復旧やメンテナンスのために、リモートセッションやリモート操作のキーボード入力は通します。
- 別のホットキーでローカル操作を復帰します。
- システムトレイからすぐ開けます。
...
LockCat が止めたいのは、PC の前にあるローカルキーボードです。メンテナンス用の入口まで閉じ込めるためのものではありません。ロック中はローカルの物理キーボード入力をブロックし、Windows がリモートデスクトップやリモート操作の注入入力として扱うキーボード入力は通します。

そのため、リモート接続で復旧したり、設定を変えたり、LockCat を終了したりできます。
